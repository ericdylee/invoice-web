...
- **Task 005: 404 에러 페이지 및 반응형 디자인**
  ...
  - 다크 모드 호환성 확인 및 기본 색상 토큰 정리

### Phase 3: 핵심 기능 구현

- **Task 006: Notion API 클라이언트 및 데이터 페칭 구현** - 우선순위
...
- **Task 007: 견적서 유효성 검증 및 에러 처리**
  ...
  - 견적서 상태가 비공개일 경우 접근 차단 로직
  - Playwright MCP로 잘못된 URL 접근, 만료된 견적서, API 실패 케이스 E2E 테스트

- **Task 008: PDF 생성 및 다운로드 기능 구현** - 우선순위
  ...
  - PDF 다운로드 API Route 또는 클라이언트 사이드 생성 로직 구현 (`src/app/api/invoice/[id]/pdf/route.ts`)
  - 다운로드 버튼 클릭 시 PDF 파일 다운로드 트리거 (파일명: `invoice-{invoice_number}.pdf`)
  - Playwright MCP로 PDF 다운로드 플로우 E2E 테스트 (파일 생성 및 다운로드 검증)

- **Task 008-1: 핵심 기능 통합 테스트**
  - Playwright MCP를 사용한 전체 사용자 플로우 테스트 (URL 접근 → 조회 → PDF 다운로드)
  - 노션 API 응답 변화 및 누락 필드에 대한 회복력 테스트
  - 404 / 에러 페이지 표시 검증
  - 모바일/태블릿/데스크톱 뷰포트별 반응형 동작 검증
  - PDF 생성 결과물의 시각적 검증 및 한글 폰트 렌더링 확인
...
- **Task 010: 배포 및 모니터링 구축**
  ...
  - 에러 모니터링 도구 연동 (예: Sentry 또는 Vercel Analytics)
